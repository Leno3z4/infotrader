"""Low-cost multi-sport + weather context for market research."""
...
SPORT_TERMS = (
    "premier league", "football", "soccer", "nba", "wnba", "nfl", "mlb", "nhl",
    "ufc", "mma", "tennis", "atp", "wta", "formula 1", "f1", "cricket", "rugby",
    "golf", "boxing", "ncaa", "college football", "champions league", "la liga",
    "bundesliga", "serie a", "ligue 1", "euroleague", "baseball", "basketball",
    "hockey", "motogp", "nascar", "indian premier league",
)
WEATHER_TERMS = (
    "weather", "temperature", "temp", "rain", "precipitation", "snow", "wind",
    "hurricane", "tornado", "storm", "forecast", "heat", "frost", "humidity",
)
...
def participants_from_question(question: str) -> list[str]:
    ...
    if len(pieces) >= 2:
        return [p.strip(" ?:-") for p in pieces[:2] if len(p.strip()) >= 2]
    return []


def _compact_team(team: dict[str, Any]) -> dict[str, Any]:
    return {
        "id": team.get("idTeam"), "name": team.get("strTeam"), "sport": team.get("strSport"),
        "league": team.get("strLeague"), "country": team.get("strCountry"), "stadium": team.get("strStadium"),
        "formed": team.get("intFormedYear"), "description": (team.get("strDescriptionEN") or "")[:800],
        "latitude": team.get("strLatitude"), "longitude": team.get("strLongitude"),
    }


def _compact_event(event: dict[str, Any]) -> dict[str, Any]:
    return {
        "id": event.get("idEvent"), "date": event.get("dateEvent"), "home": event.get("strHomeTeam"),
        "away": event.get("strAwayTeam"), "home_score": event.get("intHomeScore"),
        "away_score": event.get("intAwayScore"), "league": event.get("strLeague"),
        "status": event.get("strStatus"), "venue": event.get("strVenue"),
    }
...
async def _team_context(fetch_json: FetchJson, name: str) -> dict[str, Any] | None:
    ...
    recent = await fetch_json(f"{SPORTSDB_BASE}/eventslast.php?id={quote(str(team_id))}")
    roster = await fetch_json(f"{SPORTSDB_BASE}/lookup_all_players.php?id={quote(str(team_id))}")
    events = recent.get("results", []) if isinstance(recent, dict) else []
    ...
    return {
        ...
        "players": [
            {"name": p.get("strPlayer"), "position": p.get("strPosition"), "nationality": p.get("strNationality"),
             "description": (p.get("strDescriptionEN") or "")[:300]}
            for p in players[:12] if isinstance(p, dict)
        ],
    }
...
async def _weather_context(fetch_json: FetchJson, query: str) -> dict[str, Any] | None:
    ...
    forecast = await fetch_json(f"{FORECAST_URL}?{urlencode(params)}")
    hourly = forecast.get("hourly", {}) if isinstance(forecast, dict) else {}
    return {
        "location": {"name": place.get("name"), "country": place.get("country"), "latitude": place.get("latitude"), "longitude": place.get("longitude")},
        "current": forecast.get("current", {}) if isinstance(forecast, dict) else {},
        "hourly": {key: list(hourly.get(key, []))[:24] for key in ("time", "temperature_2m", "precipitation_probability", "precipitation", "wind_speed_10m")},
    }
...
async def build_context(fetch_json: FetchJson, question: str) -> dict[str, Any]:
    ...
    if kind == "sports":
        participants = participants_from_question(question)[:2]
        teams = []
