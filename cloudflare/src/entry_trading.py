...
def _search_markets(payload: object) -> list[dict]:
    ...
    for row in rows:
        ...
        accepting = row.get(
            "acceptingOrders",
            row.get("enableOrderBook", normalized.get("accepting_orders", False)),
        )
        normalized.update({
            ...
            "is_open": bool(row.get("active", True))
...
def _researchability_score(market: dict) -> float:
    ...
    if any(term in question for term in (
        "nba", "nfl", "mlb", "nhl", "ufc", "tennis", "atp", "wta", "premier league",
        "soccer", "football", "cricket", "formula 1"
    )):
        score += 10.0
...
async def _discover_premier_league() -> list[dict]:
    ...
    try:
        ...
        if series_id:
            ...
            structured = _search_markets(events)
...
class Default(CryptoDefault):
    async def _discover_polymarket(self) -> list[dict]:
        ...
        for query in SPORTS_QUERIES[1:] + WEATHER_QUERIES:
        ...
        for market in found:
            ...
            if not market.get("premier_league"):
                ...
                market["premier_league"] = (
                    "premier-league" in slug
                    or "premier league" in question
                )
        ...
        if len(short_horizon) >= max_markets:
            ...
            ranked = short_horizon + medium_horizon + [
                m for m in markets if m not in short_horizon and m not in medium_horizon
            ]
        ranked.sort(
            key=lambda m: (
                m.get("opportunity_score", 0),
        ...
        # Keep discovery separate from trading: selected is the best general
        # display/research set, while trade_candidates is built later from the
        # full universe so PL never disappears behind weather-heavy search results.
        return ranked
    ...
    def _decision_prompt(market: dict, research: dict) -> str:
        return json.dumps({
            ...
            "rules": [
                "PASS without a meaningful evidence-backed edge.",
                "Do not invent prices.",
                "Reduce confidence for stale news, missing lineups, injuries or weather uncertainty.",
            ],
        })
        ...
        for market in selected[:3]:
            ...
            try:
                ...
                research_results.append({
                    "subject": subject,
                    "market_kind": market.get("market_kind"),
                    "context": context,
                    **research,
                })
    ...
    async def scan_polymarket(self, store) -> dict:
        ...
        for decision_item in decisions[:3]:
            ...
            market = next(
                (m for m in trade_selected if (m.get("question") or m.get("slug")) == subject),
                None,
            )
            ...
            prompt = json.dumps({
                ...
                "trade_policy": {
                    "bucket": bucket,
                    "daily_limit": DAILY_TRADE_LIMITS[bucket],
                    "used_today": usage.get(bucket, 0),
                    "total_daily_limit": 3,
                    "total_used_today": usage.get("total", 0),
                },
                ...
                "output_schema": {
                    "action": "BUY_YES | BUY_NO | PASS",
                    "outcome": "exact outcome label",
                    "price": "0..1",
                    "size": "shares",
                    "amount_usd": "USD spend",
        ...
        return {
            ...
            "active_not_accepting_orders": sum(
                bool(m.get("active")) and not bool(m.get("closed")) and not bool(m.get("accepting_orders"))
                for m in markets
            ),
            "premier_league_markets": sum(bool(m.get("premier_league")) for m in markets),
            "short_horizon_markets": sum(
                0 < safe_float(m.get("hours_to_end") if m.get("hours_to_end") is not None else -1) <= 72
                for m in markets
            ),
            "selected": selected,
    ...
    def format_alert(self, scan: str, payload: dict) -> str:
        ...
        for item in (payload.get("trade_candidates") or [])[:5]:
            ...
            price_text = " | ".join(f"{safe_float(p):.4f}" for p in prices[:4])
            lines.append(
                ...
                f"  Liquidity: ${item.get('liquidity', 0):,.0f} | 24h vol: ${item.get('volume_24h', 0):,.0f} | Prices: {price_text or 'n/a'}"
            )
        return "\n".join(lines)[:3900]
